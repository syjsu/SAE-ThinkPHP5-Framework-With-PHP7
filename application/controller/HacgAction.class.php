<?php
	class HacgAction extends Action{
		...
		public function index(){
			...
			$page_now = "http://" . $_SERVER['SERVER_NAME'] . "/hacg/album/";

			//存入数据
			$ka = new SaeKV();
			$ret = $ka->init();
			$res = $ka->get("acg_album_all_mainindex");
		...
		public function album($url){
			...
			$ka = new SaeKV();
			$ret = $ka->init();
			$res = $ka->get("acg_album_all_".$url);
			echo '<div><div class="row">';
			//遍历返回值
			foreach ($res["info"] as $go) {
				//整合成html
				$pic1 = '<div class="col-sm-6 col-md-4"><a class="thumbnail" href="http://sinacloud.net/acg-stay/'.$go['tags'].'/'.$go['id'].'.JPG"><p>图片名称: '.$go['tags'].'</p>';
				$pic2 = '<img src="http://sinacloud.net/acg-stay/'.$go['tags'].'/'.$go['id'].'.JPG" class="img-thumbnail" > </img>';
				$pic3 = '</a></div><p></p>';
				$context = $context.$pic1.$pic2.$pic3;
			}
			echo $context;
